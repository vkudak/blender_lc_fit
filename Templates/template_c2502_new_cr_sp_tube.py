...
if __name__ == "__main__":
    ...
    # ########### SATELLITE ###
    # path to the blend
    filepath = "{{conf_res['sat_model_path']}}" #"/home/vkudak/LKD/TOPEX/Topex_size_princ_bsdf.blend" #Topex_size_SPB_glossy_bsdf.blend"
    ...
    # Solar_panel
    sat_sp1 = bpy.data.objects["Sat_sp1"] #["Cylinder"]
    sat_sp1.name = "sp_1"
    sat_sp2 = bpy.data.objects["Sat_sp2"] #["Cylinder"]
    sat_sp2.name = "sp_2"

       # Sat tube
    sat_tube = bpy.data.objects["Sat_tube"]
    sat_tube.name = "sat_tube"

    sel_objs = [sat_sp1, sat_sp2]
    for obj in sel_objs:
        obj.select_set(True)
...
##############################
    ...
    p_phase, pr_phase, pr_angle = {{var_list[0]}}, {{var_list[1]}}, {{var_list[2]}}
    cross_width, cross_lenght = {{var_list[3]}}, {{var_list[4]}}
    sp_w, sp_l = {{var_list[5]}}, {{var_list[6]}}
    tube_len, tube_d = {{var_list[7]}}, {{var_list[8]}}
    ...
    # ------------- TUBE --------------------
    sat_tube.select_set(True)
    bpy.ops.object.transform_apply(location=True, rotation=False, scale=True)
    sat_tube.dimensions = [tube_d / my_scale, tube_d / my_scale, tube_len / my_scale]

    # sp1_start_dim = sat_sp1.dimensions[0]
    # sp2_start_dim = sat_sp2.dimensions[0]
    
    # ------------CROSS----------------
    sat_cross1.dimensions = [cross_width/my_scale, cross_lenght/my_scale, 0.12/my_scale]
    sat_cross2.dimensions = [cross_lenght/my_scale, cross_width/my_scale, 0.12/my_scale]

    sel_objs = [sat_cross1, sat_cross2]
    for obj in sel_objs:
        obj.select_set(True)
    bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)

    sat_cross1.location[2] = -1 *  sat_tube.dimensions[2]/2.0 # Z
    sat_cross2.location[2] = -1 *  sat_tube.dimensions[2]/2.0 # Z
    # -------------------------------------------

    # ----------------------PANELS --------------------
    sat_sp1.dimensions = [sp_w / my_scale, sp_l / my_scale, 0.182 / my_scale]  # 0.182 - thickness
    sat_sp2.dimensions = [sp_w / my_scale, sp_l / my_scale, 0.182 / my_scale]
       # -------- SP1 -------------
    sat_sp1.select_set(True)
    bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)

    sat_sp1.location[0] = -1 * (sat_tube.dimensions[0] / 2.0 + sat_sp1.dimensions[0] / 2.0)  # X
    sat_sp1.location[2] = sat_tube.dimensions[2] / 2.0 - 0.05 * sat_tube.dimensions[2]  # Z

    # sat_sp1.select_set(True)
    # bpy.ops.object.transform_apply(location=True, rotation=False, scale=False)

       # ------- SP2 ------
    sat_sp2.select_set(True)
    bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)

    sat_sp2.location[0] = 1 * (sat_tube.dimensions[0] / 2.0 + sat_sp2.dimensions[0] / 2.0)
    sat_sp2.location[2] = sat_tube.dimensions[2] / 2.0 - 0.05 * sat_tube.dimensions[2]  # Z
    # sat_sp2.select_set(True)
    # bpy.ops.object.transform_apply(location=True, rotation=False, scale=False)
    # -----------------------------------------------------

    # --------------- MASS CENTER --------------------
    sel_objs = [sat_cross1, sat_cross2, sat_sp1, sat_sp2, sat_tube]
    for obj in sel_objs:
        obj.select_set(True)
